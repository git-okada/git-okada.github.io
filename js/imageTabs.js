    ...
    $.fn.imageTabs = function(options){
        ...
        if(conf.opentab) {
            ...
            handler.each(function() {
                if($(this).attr('href') == '#' + conf.opentab) {
                    ...
                    var onSrc = imgSrc.substr(0, imgDot) + '_active' + imgSrc.substr(imgDot, 4);
        ...
        handler.click(function() {
            ...
            if(classJudgment !== 'active') {
                // タブ画像の切り替え
                var imgSrc = $(this).children('img').attr('src').replace(/_active/g, ""); // _onが複数付かないように
                var imgSrc = $(this).children('img').attr('src').replace(/_on/g, ""); // _onが複数付かないように
                var imgDot = imgSrc.lastIndexOf('.');
                var onSrc = imgSrc.substr(0, imgDot) + '_active' + imgSrc.substr(imgDot, 4);
                $(this).children('img').attr('src',onSrc);
                var imgOff = $('ul.tab li img.active', target).attr('src').replace(/_active/g, "");
